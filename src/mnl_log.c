...
static mnl_slock_t log_locks[MAX_LOGS+1];

static int mnl_log_valid(int log_id);
 ...
 */
void mnl_log_init(void)
{
  int i;
  for (i = 0; i < MAX_LOGS+1; i++) {
    log_files[i] = NULL;
    mnl_slock_init(&log_locks[i], NULL);
  }
}
 ...
 */
int mnl_log_open(const char *path, const char* mode)
{
  int i;
  for (i = LOG_USER; i < MAX_LOGS+1; i++) {
    mnl_slock_lock(&log_locks[i]);
    if (log_files[i] == NULL) {
      log_files[i] = fopen(path, mode);
      if (log_files[i]) {
        mnl_slock_unlock(&log_locks[i]);
        return i;
      }
    }
    mnl_slock_unlock(&log_locks[i]);
...
void mnl_log_handler(const char *msg)
{
  int fd;
 ...
 * @param ... other args
 */
void mnl_log_printf0(int log_id, int log_level, const char *fmt, ...)
{
  FILE *fp;
  time_t t;
  char tmbuf[30];
  ...
  if (!mnl_log_valid(log_id)) {
    fp = stdout;
  }
  else {
    mnl_slock_lock(&log_locks[log_id]);
    if (!(fp = log_files[log_id])) {
      mnl_slock_unlock(&log_locks[log_id]);
      return;
    }
  }
